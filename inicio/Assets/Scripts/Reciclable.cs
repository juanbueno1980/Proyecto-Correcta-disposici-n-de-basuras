...
public class Reciclable : MonoBehaviour
{
    public int valor = 1;
    ...
    {
        if (collision.CompareTag("ContenedorReciclable"))
        {
            // Sumar puntos cuando colisiona con "ContenedorNoReciclable"
            gameManager.SumarPuntos(valor);
            Destroy(gameObject);
        }
        else if (collision.CompareTag("ContenedorNoReciclable") || collision.CompareTag("ContenedorOrganico"))
        {
